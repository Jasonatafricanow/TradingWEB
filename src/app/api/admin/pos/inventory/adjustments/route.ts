...
import { adjustPosInventory } from "@/services/admin/inventory-service";
import { parsePosJson, posApiErrorResponse } from "@/services/admin/pos-api-response";
import { parsePosInventoryAdjustmentRequest } from "@/services/admin/pos-contracts";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = parsePosInventoryAdjustmentRequest(await parsePosJson(request));
    const data = await adjustPosInventory({ ...body, operator });

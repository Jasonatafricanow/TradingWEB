...
import { uploadAuditBatch } from "@/services/admin/pos-audit-outbox-service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Array.isArray(body.records)) {
      throw new PosApiError("AUDIT_BATCH_REQUEST_INVALID", "records must be an array", 400);
    }
    const data = await uploadAuditBatch({ records: body.records, operator });
